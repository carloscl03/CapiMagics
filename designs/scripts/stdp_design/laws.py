...
Lo que NO es interpolable, y por que:
  ...
  L_trrd_pot   DISCRETA. LOO dejando fuera una `W` da 3.5-5.2 %; dejando fuera una `L`
...
def dvw_dep(vdep, W_trrd_dep, ncw=C.NCW_REF):
    """Cambio de peso [V] por evento anticausal, en funcion de la traza.

    NEGATIVO (deprime). `vdep` en [0.50, 1.00] V, `W_trrd_dep` en la CAJA_W4.
    LOO por geometria 3.05 %, peor 4.51 %. Familia elegida por parsimonia:
    empata en peor caso con una de 4 coeficientes usando 3.
    """
    c = [_cuad_lnW(b, W_trrd_dep) for b in C.NUC_DEP]
    s = exp(c[0] * vdep + c[1] * log(vdep) + c[2])
    return -s * _escala_cw(ncw)


def dvw_pot(vtr, W_trrd_pot, L_trrd_pot, ncw=C.NCW_REF):
    """Cambio de peso [V] por evento causal. POSITIVO (potencia).

    `vtr` es la PROFUNDIDAD de la traza: `3.3 - vpot`, en [0.70, 1.30] V.
    `L_trrd_pot` tiene que ser uno de `L_TRRD_POT_DISC`: no se interpola (43-47 % de error).
    LOO por geometria 3.5-5.2 %.
    """
    if L_trrd_pot not in C.L_TRRD_POT_DISC:
        raise ValueError("L_trrd_pot = %s no es interpolable; usa una de %s"
                         % (L_trrd_pot, C.L_TRRD_POT_DISC))
    B = C.NUC_POT[C.L_TRRD_POT_DISC.index(L_trrd_pot)]
    c = [_cuad_lnW(b, W_trrd_pot) for b in B]
    s = exp(c[0] * vtr ** 3 + c[1] * vtr ** 2 + c[2] * vtr + c[3])
    return s * _escala_cw(ncw)


# --- el suelo: el termino NO HEBBIANO ---------------------------------------
def suelo_dep(W_trrd_dep, ncw=C.NCW_REF):
    ...
    con `W_trrd_dep`. Depende SOLO del pre, asi que el peso deriva con actividad
    ...
    """
    return (C.SUELO_DEP[0] * W_trrd_dep + C.SUELO_DEP[1]) * _escala_cw(ncw)


def suelo_pot(W_trrd_pot, ncw=C.NCW_REF):
    """Idem por spike POST [V]. NO depende de `L_trrd_pot` (identico a 4 cifras).

    Aqui los terminos tienen signo opuesto, asi que **se anula solo** en
    `W_trrd_pot = 0.357 um`. Es el remedio limpio; la depresion no lo tiene.
    """
    return (C.SUELO_POT[0] * W_trrd_pot + C.SUELO_POT[1]) * _escala_cw(ncw)


def w1_suelo_nulo():
    """La `W_trrd_pot` que anula el suelo de potenciacion [um]."""
...
# --- cajas ------------------------------------------------------------------
def en_caja_dep(W_trrd_dep, vdep):
    return (C.CAJA_W4[0] <= W_trrd_dep <= C.CAJA_W4[1]) and (0.50 <= vdep <= 1.00)


def en_caja_pot(W_trrd_pot, vtr):
    return (C.CAJA_W1[0] <= W_trrd_pot <= C.CAJA_W1[1]) and (0.70 <= vtr <= 1.30)
