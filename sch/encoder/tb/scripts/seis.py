...
EXT =np.array([[0.220,1.796],[0.300,2.699],[0.280,0.620],[0.300,3.781]])
LO,HI=np.log10(EXT[:,0]),np.log10(EXT[:,1])
rng=np.random.default_rng(404); N=800
G=10**rng.uniform(LO,HI,(N,4)); np.save('seis_geos.npy',G)
...
for t in range(0,N,200):
    ...
    L+=['.control','dc VDIF -0.6 0.6 0.02','wrdata s6_%d.dat '%t+' '.join(vec),'.endc','.end']
    open('s6_%d.spice'%t,'w').write('\n'.join(L)+'\n')
    subprocess.run(['ngspice','-b','s6_%d.spice'%t],capture_output=True)
    A=np.loadtxt('s6_%d.dat'%t); vd=A[:,0]
    ...
    print('  trozo %d'%t)
R=np.array(R); np.savez('seis.npz',geos=G,res=R,LD=LD,W9=W9,ext=EXT,orig=ORIG)
