...
GE=np.zeros((N,6)); GE[:,1]=np.log10(LD); GE[:,4]=np.log10(W9)
GE[:,J]=rng.uniform(LO[J],HI[J],(N,4))
GE=10**GE; np.save('cinco_geos.npy',GE)
...
for t in range(0,N,TROZO):
    ...
    L+=['.control','dc VDIF -0.6 0.6 0.02','wrdata c5_%d.dat '%t+' '.join(vec),'.endc','.end']
    open('c5_%d.spice'%t,'w').write('\n'.join(L)+'\n')
    subprocess.run(['ngspice','-b','c5_%d.spice'%t],capture_output=True)
    A=np.loadtxt('c5_%d.dat'%t); vd=A[:,0]
...
R=np.array(R); np.savez('cinco.npz',geos=GE,res=R,LD=LD,W9=W9)
