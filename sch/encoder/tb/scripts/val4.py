...
D=np.load('cuatro.npz'); GE=D['geos']; R=D['res']; LD=float(D['LD']); W9=float(D['W9'])
J=[0,2,3,5]
m=(R[:,3]>0.15)&np.isfinite(R).all(1)&(R[:,0]>0)
X=np.log10(GE[m][:,J]); LO,HI=X.min(0),X.max(0)
...
NG=np.zeros((25,6)); NG[:,1]=np.log10(LD); NG[:,4]=np.log10(W9)
NG[:,J]=rng.uniform(LO,HI,(25,4)); NG=10**NG
...
for Ia,Ib,g,_ in ped:
    ...
    sol.append(x)
S=np.zeros((len(sol),6)); S[:,1]=np.log10(LD); S[:,4]=np.log10(W9); S[:,J]=np.array(sol)
got=sim(10**S,'s4')
